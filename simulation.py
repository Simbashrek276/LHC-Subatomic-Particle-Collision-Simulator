...
def collision():
    ...
    We pick a final state, then hand out energy and momentum. The first n-1
    particles get random energy from whatever budget is left, and the last
    particle takes the remainder so the total energy stays at TOTAL_ENERGY.
    Its momentum is set to balance the others so the momenta sum to zero.
    """
    ...
    # the last particle gets the leftover energy, and momentum that cancels the rest.
...
def detector(event, f, collision_number, logged_so_far):
    ...
    # Show every collision we simulated, passed or failed.
    cut_tag = "PASSED CUTS" if passes_cuts else "FAILED CUTS"
    print(f"___COLLISION: {collision_number}___ [{cut_tag}]")
    for line in body:
        print(line, end="")
    print()
